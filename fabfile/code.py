...
from nginx import touch_WSGI_script
...
def make_website_skeleton():
    ...
    """
    require('web_root', 'app_domain', 'web_user', 
                provided_by=('staging', 'production'))
    puts("Setting up initial filesystem layout for this instance..." % env)
    with hide('commands'):
        with cd(env.web_root):
            if files.exists(env.app_domain):
                ...
                if not console.confirm(confirmation_msg):
                    ...
                    # re-create a SSH account for ``WEB_USER``
...
#            sudo('mkdir -p %(app_domain)s/private/apache' % env)
            sudo('mkdir -p %(app_domain)s/private/nginx' % env)
    ...
    with hide('commands'):         
        ...
        with lcd(env.local_project_root):
            with cd(env.project_root):
                ...
                settings_file = '%(project)s/settings_%(environment)s.py' % env
                put(settings_file, settings_file, mode=0644)
                # update Django main URLconf module
                urls_file = '%(project)s/urls.py' % env        
...
##                source = 'wsgi_%(environment)s.py' % env
##                dest = source
